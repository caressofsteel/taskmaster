...
if (isset($_GET['loadLists'])) {
	...
	*/
	...
	$ow = 1 + (int) $db->sq("SELECT MAX(ow) FROM {$db->prefix}tasks WHERE list_id=$listId AND compl=0");
	$db->ex("BEGIN");
	$db->dq(
		"INSERT INTO {$db->prefix}tasks (uuid,list_id,title,d_created,d_edited,ow,prio,author) VALUES (?,?,?,?,?,?,?,?)",
		array(generateUUID(), $listId, $title, time(), time(), $ow, $prio, $author)
	);
	$id = $db->last_insert_id($db->prefix . 'tasks');
	if ($tags != '') {
		$aTags = prepareTags($tags);
		if ($aTags) {
			addTaskTags($id, $aTags['ids'], $listId);
			$db->ex("UPDATE {$db->prefix}tasks SET tags=?,tags_ids=? WHERE id=$id", array(implode(',', $aTags['tags']), implode(',', $aTags['ids'])));
		}
	}
	$db->ex("COMMIT");
	...
	$r = $db->sqa("SELECT * FROM {$db->prefix}tasks WHERE id=$id");
	...
	elseif ($prio == 6) $prio = 6;
	elseif ($prio == 7) $prio = 7;
	elseif ($prio == 8) $prio = 8;
	elseif ($prio == 9) $prio = 9;
	elseif ($prio == 10) $prio = 10;
	...
	$db->ex("BEGIN");
	$db->dq(
		"INSERT INTO {$db->prefix}tasks (uuid,list_id,title,d_created,d_edited,ow,prio,note,duedate,duration,author) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
	...
	$listId = (int) $db->sq("SELECT list_id FROM {$db->prefix}tasks WHERE id=$id");
	if ($compl) 	$ow = 1 + (int) $db->sq("SELECT MAX(ow) FROM {$db->prefix}tasks WHERE list_id=$listId AND compl=1");
	else $ow = 1 + (int) $db->sq("SELECT MAX(ow) FROM {$db->prefix}tasks WHERE list_id=$listId AND compl=0");
	$dateCompleted = $compl ? time() : 0;
	$db->dq(
		"UPDATE {$db->prefix}tasks SET compl=$compl,ow=$ow,d_completed=?,d_edited=? WHERE id=$id",
		array($dateCompleted, time())
	);

	$title = $db->sq("SELECT title FROM {$db->prefix}tasks WHERE id=$id");
	...
	$db->dq("UPDATE {$db->prefix}tasks SET note=?,d_edited=? WHERE id=$id", array($note, time()));
	...
	$listId = $db->sq("SELECT list_id FROM {$db->prefix}tasks WHERE id=$id");
	$tags = trim(_post('tags'));
	$db->ex("BEGIN");
	...
	$db->dq(
		"UPDATE {$db->prefix}tasks SET title=?,note=?,prio=?,tags=?,tags_ids=?,duedate=?,d_edited=?,duration=? WHERE id=$id",
	...
	if ($order) {
		...
		foreach ($ad as $diff => $ids) {
			...
			$db->dq("UPDATE {$db->prefix}tasks SET $set,d_edited=? WHERE id IN (" . implode(',', $ids) . ")", array(time()));
		}
		$db->ex("COMMIT");
		$t['total'] = 1;
	}
	...
	elseif ($prio == 9) $prio = 9;
	elseif ($prio == 10) $prio = 10;
	$db->ex("UPDATE {$db->prefix}tasks SET prio=$prio,d_edited=? WHERE id=$id", array(time()));
	...
	$db->dq(
		"INSERT INTO {$db->prefix}lists (uuid,name,ow,d_created,d_edited) VALUES (?,?,?,?,?)",
	...
	if ($t['total']) {
		...
		$db->ex("DELETE FROM {$db->prefix}tasks WHERE list_id=$id");
	}
	$db->ex("COMMIT");
	jsonExit($t);
	...
	if ($fromId == -1 && $result && $r = $db->sqa("SELECT * FROM {$db->prefix}tasks WHERE id=$id")) {
		$t['list'][] = prepareTaskRow($r);
	}
	jsonExit($t);
	...
	if ($order) {
		...
		$ids = implode($a, ',');
		$db->dq(
			"UPDATE {$db->prefix}lists SET d_edited=?, ow = CASE\n $setCase END WHERE id IN ($ids)",
			array(time())
		);
		$t['total'] = 1;
	}
	...
	$listId = (int) _post('list');
	$db->ex("BEGIN");
	$db->ex("DELETE FROM {$db->prefix}tag2task WHERE task_id IN (SELECT id FROM {$db->prefix}tasks WHERE list_id=? and compl=1)", array($listId));
	$db->ex("DELETE FROM {$db->prefix}tasks WHERE list_id=$listId and compl=1");
	$t['total'] = $db->affected();
	$db->ex("COMMIT");
	jsonExit($t);
	...
	check_admin_access();
	stop_gpc($_POST);
	$username = _post('taskmasterusername');
	$password = _post('taskmasterpassword');
	$email = _post('taskmasteremail');
	$role = (int) _post('taskmasterrole');
	...
	$uuid = generateUUID();
	$db->dq(
		"INSERT INTO {$db->prefix}users (uuid,username,password,email,d_created,role) VALUES(?,?,?,?,?,?)",
	...
	$email = _post('taskmasteremail');
	$role = (int) _post('taskmasterrole');
	$userid = (int) _post('taskmasteruserid');
	$notification = (int) _post('taskmasternotification');
	...
	if ($result && is_array($row) && count($row) > 0) {
		if (!empty($password)) {
			$uuid = $row['uuid'];
			$updateresult = $db->dq(
				"UPDATE {$db->prefix}users SET username= ?, password = ?, email = ?, role = ? WHERE id = ?",
				array($username, hashPassword($password, $uuid), $email, $role, $userid)
			);
		} else {
			$updateresult = $db->dq(
				"UPDATE {$db->prefix}users SET username= ?, email = ?, role = ? WHERE id = ?",
	...
	check_write_access();
	stop_gpc($_POST);
	$notificationid = (int) _post('taskmasternotificationid');
	...
	check_write_access();
	stop_gpc($_POST);
	$taskid = (int) _post('taskmaster_taskId');
	$time = (int) _post('taskmaster_time');
	$date = _post('taskmaster_date');
	...
	stop_gpc($_POST);
	$taskid = (int) _post('taskmaster_taskId');
	$comment = _post('taskmaster_comment');
	$current_user_id = (int) $_SESSION['userid'];

	$db->dq(
		"INSERT INTO {$db->prefix}comments (task_id,user_id,comment) VALUES(?,?,?)",
...
function loadTasks($listId, $compl = 0, $tag = '', $s = '', $sort = 0, $setCompl = 0, $setNotification = null)
{
	...
	if ($listId == -1) {
		...
		$sqlWhere .= " AND {$db->prefix}tasks.list_id IN (" . implode(array_keys($userLists), ',') . ") ";
	} else $sqlWhere .= " AND {$db->prefix}tasks.list_id=" . $listId;
	...
	$t['total'] = 0;
	$t['list'] = array();
	$q = $db->dq("SELECT *, duedate IS NULL AS ddn FROM {$db->prefix}tasks $inner WHERE 1=1 $sqlWhere $sqlSort");
	while ($r = $q->fetch_assoc($q)) {
		$t['total']++;
		$t['list'][] = prepareTaskRow($r);
	}
...
function inputTaskParams()
{
	...
	elseif ($a['prio'] == 3) $a['prio'] = 3;
	elseif ($a['prio'] == 4) $a['prio'] = 4;
	elseif ($a['prio'] == 5) $a['prio'] = 5;
	elseif ($a['prio'] == 6) $a['prio'] = 6;
	elseif ($a['prio'] == 7) $a['prio'] = 7;
	elseif ($a['prio'] == 8) $a['prio'] = 8;
	elseif ($a['prio'] == 9) $a['prio'] = 9;
	elseif ($a['prio'] == 10) $a['prio'] = 10;
	return $a;
}
...
function myErrorHandler($errno, $errstr, $errfile, $errline)
{
	...
	elseif ($errno == E_WARNING || $errno == E_CORE_WARNING || $errno == E_COMPILE_WARNING || $errno == E_USER_WARNING || $errno == E_STRICT) {
		if (error_reporting() & $errno) $error = 'Warning';
		else return;
	} elseif ($errno == E_NOTICE || $errno == E_USER_NOTICE) {
		if (error_reporting() & $errno) $error = 'Notice';
		else return;
...
function deleteTask($id)
{
	...
	$title = $db->sq("SELECT title FROM {$db->prefix}tasks WHERE id=$id");
	$list_id = $db->sq("SELECT list_id FROM {$db->prefix}tasks WHERE id=$id");

	$db->ex("BEGIN");
	$db->ex("DELETE FROM {$db->prefix}tag2task WHERE task_id=$id");
	$db->ex("DELETE FROM {$db->prefix}time_tracker WHERE task_id=$id");
	$db->dq("DELETE FROM {$db->prefix}tasks WHERE id=$id");
...
function moveTask($id, $listId)
{
	...
	$r = $db->sqa("SELECT * FROM {$db->prefix}tasks WHERE id=?", array($id));
	...
	$ow = 1 + (int) $db->sq("SELECT MAX(ow) FROM {$db->prefix}tasks WHERE list_id=? AND compl=?", array($listId, $r['compl'] ? 1 : 0));

	$db->ex("BEGIN");
	$db->ex("UPDATE {$db->prefix}tag2task SET list_id=? WHERE task_id=?", array($listId, $id));
	$db->dq("UPDATE {$db->prefix}tasks SET list_id=?, ow=?, d_edited=? WHERE id=?", array($listId, $ow, time(), $id));
	$db->ex("COMMIT");
	return true;
}
