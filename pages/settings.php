...
// Alternate Theme Approach - Needs to be updated
function getStyles($withContents = 0)
{
	if (!$h = opendir(TASKMASTERPATH . 'themes/default')) return false;
	$a = array();
	while (false !== ($file = readdir($h))) {
		if (preg_match('/(.+)\.css$/', $file, $m) && $file != 'style.css') {
			$a[$m[1]] = $m[1];
			if ($withContents) {
				$a[$m[1]] = getLangDetails(TASKMASTERPATH . 'lang' . DIRECTORY_SEPARATOR . $file, $m[1]);
...
{
	if (!$h = opendir(TASKMASTERPATH . 'lang')) return false;
	$a = array();
	while (false !== ($file = readdir($h))) {
...
function getLangDetails($filename, $default)
{
	...
	if (preg_match("|/\\*\s*# Taskmaster([\s\S]+?)\\*/|", $contents, $m)) {
		$str = $m[1];
...
<form id="settings_form" method="post" action="settings.php">
	...
	<table class="taskmaster-settings-table">

		<tr>
			<th><?php _e('set_template'); ?>:<br /></th>
			<td>
				<select name="template"><?php echo selectOptions(array('crimson' => __('Crimson'), 'forest' => ('Forest'), 'samhain' => __('Samhain'), 'sea' => __('Sea'), 'sky' => __('Sky'), 'tuxedo' => ('Tuxedo'), 'yule' => __('Yule')), _c('template')); ?></select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<label><input type="radio" name="allowpassword" value="1" <?php if (_c('password') != '' && _c('multiuser') == 0) echo 'checked="checked"'; ?> onclick='$(this.form).find("input[name=password]").attr("disabled",false);$(this.form).find("a.taskmaster-manage-users").hide()' /><?php _e('set_enabled_single'); ?></label> <br />
				<label><input type="radio" name="allowpassword" value="2" <?php if (_c('multiuser') == 1) echo 'checked="checked"'; ?> onclick='$(this.form).find("input[name=password]").attr("disabled","disabled");$(this.form).find("a.taskmaster-manage-users").show()' /><?php _e('set_enabled_multi'); ?> <a href="#manageusers" id="manageusers" class="taskmaster-manage-users" <?php if (_c('multiuser') == 0) echo ' style="display:none"'; ?>><?php _e('set_manage_users'); ?></a></label> <br />
				<label><input type="radio" name="allowpassword" value="0" <?php if (_c('password') == '' && _c('multiuser') == 0) echo 'checked="checked"'; ?> onclick='$(this.form).find("input[name=password]").attr("disabled","disabled");$(this.form).find("a.taskmaster-manage-users").hide()' /><?php _e('set_disabled'); ?></label> <br />
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<label><input type="radio" name="session" value="files" <?php if (_c('session') == 'files') echo 'checked="checked"'; ?> /><?php _e('set_sessions_files'); ?></label> <span class="descr">(&lt;taskmaster_dir&gt;/tmp/sessions)</span>
			</td>
		</tr>
			...
			<td>
				...
				<select onchange="if(this.value!=0) this.form.dateformat.value=this.value;">
					<?php echo selectOptions(array(
						'F j, Y' => formatTime('F j, Y'), 'M d, Y' => formatTime('M d, Y'), 'j M Y' => formatTime('j M Y'), 'd F Y' => formatTime('d F Y'),
						'n/j/Y' => formatTime('n/j/Y'), 'd.m.Y' => formatTime('d.m.Y'), 'j. F Y' => formatTime('j. F Y'), 0 => __('set_custom')
					), _c('dateformat'), 0); ?>
				</select>
			</td>
		</tr>
			...
			<td>
				...
				<select onchange="if(this.value!=0) this.form.dateformat2.value=this.value;">
					<?php echo selectOptions(array(
						...
						'd/m/y' => 'dd/mm/yy (' . date('d/m/y') . ')', 0 => __('set_custom')
					), _c('dateformat2'), 0); ?>
				</select>
			</td>
		</tr>
		...
		<tr>
			<td colspan="2" class="form-buttons">
				...
				<input type="button" class="taskmaster-back-button" value="<?php _e('set_cancel'); ?>" />
