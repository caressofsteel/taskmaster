...
var taskmaster = window.taskmaster = _taskmaster = {
	...
	init: function(options)
	{
        ...
        $('#priopopup .prio-pos-5').click(function(){
            prioClick(5,this);
        });

        $('#priopopup .prio-bug').click(function(){
            prioClick(6,this);
        });

        $('#priopopup .prio-work').click(function(){
            prioClick(7,this);
        });

        $('#priopopup .prio-review').click(function(){
            prioClick(8,this);
        });

        $('#priopopup .prio-note').click(function(){
            prioClick(9,this);
        });

        $('#priopopup .prio-mile').click(function(){
            prioClick(10,this);
        });
			...
			if(res && res.total)
			{
				...
				$.each(res.list, function(i,item){
                    ...
                    if(parseInt(item.private) == 0 || parseInt(item.private) == parseInt(flag.userId)) {
						...
						/*if(!openArchivedListId && _taskmaster.show_archived_lists && item.archive == 1) {
	...
	_taskmaster.db.request('addList', {name:r}, function(json){
		...
		if(i > 0) {
			$('#lists ul').append('<li id="list_'+item.id+'" class="taskmaster-tab">'+
					...
					'<div class="list-action"></div></a></li>');
			taskmaster.doAction('listAdded', item);
		}
...
function renameCurList()
{
	...
	_taskmaster.db.request('renameList', {list:curList.id, name:r}, function(json){
		...
		taskmaster.doAction('listRenamed', item);
	});
};
...
{
	var cl =''; var v = '';
	if(prio < 0) { cl = 'prio-neg prio-neg-'+Math.abs(prio);  v = '<i style="margin-left: 3px;" class="fa fa-lock" /> '} //v = '&#8722;'+Math.abs(prio); }	// &#8722; = &minus; = −
    else if(prio == 0) { cl = 'prio-zero'; v = '&#177;0'; } // &#177; = &plusmn; = ±
    else if(prio == 1 || prio == 2 || prio == 3 || prio == 4 || prio == 5) { cl = 'prio-pos prio-pos-'+prio; v = '+'+prio; }
    else if (prio == 6) { cl = 'prio-bug', v = '<i style="margin-left: 3px;" class="fa fa-bug" /> '}
    else if (prio == 7) { cl = 'prio-work', v = '<i style="margin-left: 4px; margin-right: 0px;" class="fa fa-gear" /> '}
    else if (prio == 8) { cl = 'prio-review', v = '<i style="margin-left: 3px;" class="fa fa-flag-checkered" /> '}
    else if (prio == 9) { cl = 'prio-note', v = '<i style="margin-left: 3px;" class="fa fa-info-circle" /> '}
    else { cl = 'prio-mile', v = '<i style="margin-left: 3px;" class="fa fa-trophy  " /> '}
...
function prioPopup(act, el, id)
{
    ...
    // PRIORITY OFFSET
	priopopup.css({ position: 'absolute', top: offset.top-6, left: offset.right - 5 - (prio*20) });
	objPrio.taskId = id;
	objPrio.el = el;
...
function tabSelect(elementOrId)
{
	...
	if(curList.id != id)
	{
		...
		taskmaster.doAction('listSelected', tabLists.get(id));
	}
...
{
	if(!taskmaster.menus.listMenu) taskmaster.menus.listMenu = new taskmasterMenu('listmenucontainer', {onclick:listMenuClick});
	taskmaster.menus.listMenu.show(el);
};
...
function doAuth(form)
{
	if(flag.multiUser)
    {
        $.post(taskmaster.taskmasterUrl+'ajax.php?login', { login:1, username: form.username.value, password: form.password.value }, function(json){
            form.password.value = '';
            form.username.value = '';
            if(json.logged)
            {
    ...
    {
        $.post(taskmaster.taskmasterUrl+'ajax.php?login', { login:1, password: form.password.value }, function(json){
            form.password.value = '';
            if(json.logged)
            {
                flag.isLogged = true;
...
function logout()
{
	$.post(taskmaster.taskmasterUrl+'ajax.php?logout', { logout:1 }, function(json){
		flag.isLogged = false;
		window.location.reload();
	}, 'json');
	return false;
